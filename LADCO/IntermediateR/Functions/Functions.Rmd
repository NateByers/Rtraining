...
will cover how to create our own functions. We'll also review a some looping functions,
...
For example, if we want to use the `mean()` functions, we look at the documentation
by typing `?mean()`.
...
contain the parameters want our function to have, and the curly braces will contain the 
...
So we've found how many numbers are in the `x` vector, we've added up all the values
...
When we write a function, it's usually not as straight forward as this example. In fact, 
you may notice that there could be problems using our function. What's to stop us from
trying to use a character vector, or a data frame, as `x`?
...
# MARGIN = 2 means we are applying the max() function to each column
# na.rm is being passed to the max() function
air_max <- apply(air, MARGIN = 2, FUN = max, na.rm = TRUE)
air_max
```
