...
`dplyr` is written by <a href="http://had.co.nz/">Hadley Wickham</a> and can be
...
x \| y |	x OR y
x & y	| x AND y
x %in% y | compare consecutive elements of x to every element of y
...
`dplyr` takes advantage of the pipe operator, `%>%`. This operator allows you 
to string `dplyr` functions together, without having to assign intermediate data
frames. For example, let's say that we wanted to select 
