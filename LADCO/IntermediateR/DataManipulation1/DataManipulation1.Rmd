...
- [summarize()](#summarize), summarizing rows
...
# Summarize rows with group_by() and summarize() {#summarize}

Suppose we want to summarize the value for each day in `airdata`. The first thing we need to do is create a date column, so we use another handy-Hadley package, `stringr`. The `str_split_fixed` function will split a character vector and return a fixed number of columns.
...
In base R, we can summarize the mean of each day by using the `tapply()` function (see `?tapply()`). Let's find the daily mean for a given site and a given parameter.
...
The `summarize()` function will split up the column by grouping together cells that have
common factor levels in the site, parameter, and date columns.

```{r}
d_daily_mean <- summarize(d_daily_mean, daily_mean = mean(value, na.rm = FALSE))
head(d_daily_mean, 3)
```

The resulting [data frame](#classes) has just three columns: site, parameter, date, and daily_mean. Instead
of subsetting down to one site and one parameter before calculating the daily mean, `summarize()` will take
