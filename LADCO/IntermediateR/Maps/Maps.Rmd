...
nonattain <- read.table(header=T, text='
      state  areas
   illinois      2
    indiana      2
   michigan      0
...
Now we'll use the `choroplethr` package to make a heat map. The `choroplethr()` 
...
Now we can use the `choroplethr()` function. 
...
Go to the <a href="http://rpubs.com/NateByers/MapsExercises" target="_blank">exercises page</a> for this tutorial and test your comprehension of this material.
...
It is possible to perform any kind of geographical analysis you would like in R.
The `sp` package is the foundational R package for working with spatial data.
...
Now we can create `SpatialPoints` data objects by using the `SpatialPoints()` 
function. 
...
Below is a list of resources for learning more about what you can do with R and geographic data.
