...
<a href="http://rpubs.com/NateByers/maps" target="_blank">Maps</a> tutorial. 
...
4. Use `choroplethr` to make a heat map of the number of NO~2~ monitors
in each state.

[Solution](#ex4)

5. Use `googleVis` to make a Google map of the Ohio monitors.

[Solution](#ex5)

6. Use `leaflet` to make a Leaflet map of the Ohio monitors.
...
Make a summary table of the number of NO~2~ monitors in each state.

```{r}
monitors <- group_by(no2_annual, State.Name)
monitors <- summarize(monitors, value = n())
```

Make a column named "region" that is all lowercase state names and has a `character`
class. Remove non-states.

```{r}
monitors$region <- as.character(tolower(monitors$State.Name))
monitors <- filter(monitors, region != "puerto rico")
```
...
```{r, message=FALSE, warning=FALSE}
library(choroplethr)
no2_map <- state_choropleth(monitors[, c("region", "value")],
                                  legend = "NO2 Monitors")
no2_map
```
