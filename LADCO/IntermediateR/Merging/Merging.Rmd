...
column is `state` and `State.Code` respectively.
...
The default operation of `merge()` is an inner join. This means that, in column(s) in 
common between the two data frames, only records that match will be kept. In the example
