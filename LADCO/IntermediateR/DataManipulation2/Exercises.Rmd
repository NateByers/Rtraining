...
<a href="http://rpubs.com/NateByers/datamanip2" target="_blank">Data Manipulation 2</a> tutorial. 
...
5. Reshape the monthly data from exercise 4 to a long format and plot it using
`ggplot()`.
...
```{r}
air_monthly_long <- gather(air_monthly, parameter, value, ozone:solar)
library(ggplot2)
p <- ggplot(air_monthly_long, aes(month, value)) 
p + geom_line() + facet_grid(parameter ~., scales = "free")
```
